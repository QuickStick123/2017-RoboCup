...
int speedMultiply = 10; //Pwm multiplyer to ajust the spped of the robot for easy testing
...
void i2cPacket(int howMany) {
  ...
  correctDirection = Wire.read(); // Read weather to correct left, right
  correctAmmount = Wire.read(); // Read the ammount to change the 
  kicker = Wire.read();
