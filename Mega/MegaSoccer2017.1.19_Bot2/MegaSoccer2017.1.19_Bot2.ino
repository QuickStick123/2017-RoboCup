...
void nanoCom() {
  ...
  Wire.write(directionSend * motorsOn); // sends one byte DirectionSend
  if (Straight == HIGH) {
...
void detectColor() {
  if (ColourIN >= 0 && ColourIN < 400 && onWhite != HIGH) { //White
  ...
  else if (ColourIN >= 400 && ColourIN <= 2000 && onDarkGreen != HIGH) { //Dark Green
    colorSensorError = LOW;
    onWhite = LOW;
    onDarkGreen = HIGH;
    onBlack = LOW;
...
void haveBallLogic() {
  ...
  if (DirectionGo == 1) {
    ...
    if (kickerForwardsCount == 4) {
      ...
      Serial.print("NotForwards\t");
      kickerForwardsCount = 0;
    }
  }
  if (DirectionGo == 1 && kickerForwardTimer < millis() - 500) {
    if (pixyX >= 158 && pixyX <= 162 && digitalRead(23) == LOW && frontUltraSonic > 10 && motorsOn && calibratedCompassValue <= 90 && calibratedCompassValue >= -90) {
      kickerForwardTimer = millis();
      kickerForwardReset = true;
      kicker = 27;
      nanoCom();
...
void CorrectDirection () {
  if (haveBall && calibratedCompassValue <= 90 && calibratedCompassValue >= -90) {
