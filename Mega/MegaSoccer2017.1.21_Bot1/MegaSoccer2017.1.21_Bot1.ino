...
void nanoCom() {
  ...
  //5 = Back Right
  //6 = Back
  if (pulseTimeTotal < 400 && haveBall == false && onWhite == false) {
    DisableMode = false;
  }
  else if (700 < pulseTimeTotal || onWhite == true) {
    DisableMode = true;
  }
...
void IRCalc() {
  ...
  */
  ...
  for (int z = 0; z <= 13; z++) {
    if (z == 11 || z == 10 || z == 9) {
      pulseTimeSum = pulseTimeSum + ((pulseTime[z][0] + pulseTime[z][1] + pulseTime[z][2]) / 3) * 2.5;
    }
    else {
      pulseTimeSum = pulseTimeSum + (pulseTime[z][0] + pulseTime[z][1] + pulseTime[z][2]) / 3;
    }
  }
  pulseTimeTotal = pulseTimeSum;
  for (int c = 0; c <= 13; c++) {
...
void onWhiteFunc() {
  if (onWhite == HIGH) { //if on white
    ...
    if (frontUltraSonic < 33 && lastBounceFB == false) {
      bounceFB = HIGH;
      verifiedFieldSideFrontBack = 'F';
    }
    else if (backUltraSonic < 33 && lastBounceFB == false) {
      bounceFB = HIGH;
      verifiedFieldSideFrontBack = 'B';
    }
    disableMotorsButton();

    if (bounceFB == HIGH && lastBounceFB == false ) {
