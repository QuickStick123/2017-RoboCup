...
int ColourDisableDirection(int targetDirection){
  //if(disableDirectionTimer > millis()-2500){
    if(targetDirection == 1){//Forwards
      if(disableDirection == 1 || disableDirection == 2 || disableDirection == 3){  
        disableDirectionCount = 0;
        return 0;
      }
      else {
        if(disableDirectionCount >= 5 || disableDirection == 0){
          disableDirection = 0;
          disableDirectionCount = 0;
          return targetDirection;   
        }
        else{
          disableDirectionCount++;
          return 0;
        }
      }
    }
...
void nanoCom() {
  ...
  if(onWhite == true){
    Serial.print("ONWHITE");
    Wire.write(DirectionGo * motorsOn);
    Serial.print(DirectionGo);
  }
  else{
...
void detectColor() {
  ...
  else if (ColourIN >= 500 && ColourIN < 2000 && onDarkGreen != HIGH) { //Dark Green
    colorSensorError = LOW;
    onWhite = LOW;
    onDarkGreen = HIGH;
    onBlack = LOW;
  ...
  else if (ColourIN >= 2000 && ColourIN <= 2500 && onBlack != HIGH) { //Black
    colorSensorError = LOW;
    onWhite = LOW;
    onDarkGreen = LOW;
    onBlack = HIGH;
...
void haveBallLogic() {
  ...
  void haveBallCalc() {
    ...
    Serial.print(laser2On);
    if(laser1On == false || laser2On == false){ //if laser 1 or 2 is broken. pulseTime[13] > 350 || 
