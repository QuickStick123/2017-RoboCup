  ...
  if (onWhite == HIGH) { //if on white
    ...
    if (bounceFB == HIGH ) {
      if (verifiedFieldSideFrontBack == 'F') {
        ...
        if (verifiedFieldSide == 'L') {
          delayOnWhite(50, 5); //Goes BackRight For 150 milliseconds
        }
        else {
          delayOnWhite(50, 4); //Goes BackLeft For 150 milliseconds
        }
        bounceFB = LOW;
      }
        ...
        if (verifiedFieldSide == 'L') {
          delayOnWhite(50, 3); //Goes ForwardRight For 150 milliseconds
        }
        else {
          delayOnWhite(50, 2); //Goes BackLeft For 150 milliseconds
        }
        bounceFB = LOW;
      }
    }
...
void haveBallLogic() {
  ...
  if (DirectionGo == 1 && kickerForwardTimer < millis() - 500) {
    if (pixyX >= 158 && pixyX <= 162 && digitalRead(23) == LOW && frontUltraSonic > 10 && motorsOn) {
      kickerForwardTimer = millis();
      kickerForwardReset = true;
      kicker = 27;
      nanoCom();
...
void haveBallCalc() {
  ...
  /////abcdefg123Serial.print(laser2On);
  if (laser1On == false && (pulseTime[13][2] + pulseTime[13][1] + pulseTime[13][0]) / 3 > 300 && laser2On == false) { //if sensor 14 or 15 is getting a reading. pulseTime[13] > 350 ||
