...
void transformation(float uncalibrated_values[3]) {
  ...
  {
    {1.307, 0.039, 0.031}, //M11-M13
    {0.018, 1.287, -0.042}, //M21-M23
    {0.018, -0.016, 1.404} //M31-M33
  };
  //bias[3] is the bias
...
int pulseTimeSum[2];
int pulseTimeTotal;
int pulseTimeFrontTotal;
int pulseTimeBackTotal;
int pulsePin[16];  //Hold TSOP pin number
volatile unsigned long pulseTime[14][3];//Length of TSOP pulse, No pulse = 0.
...
boolean onBlack = LOW;
volatile boolean onWhitecalc = LOW;
int pastOnWhite = 3;
...
void loop() {
  // Serial.print(verifiedFieldSideFrontBack);
  ...
  //  Serial.println();
  Serial.print("\tpulseTimeFrontTotal: ");
  Serial.print(pulseTimeFrontTotal);
  Serial.print("\tpulseTimeBackTotal: ");
  Serial.print(pulseTimeBackTotal);
  Serial.print("\tpulseTimeTotal: ");
  Serial.print(pulseTimeTotal);
  Serial.print("\t");
  for (int c = 0; c <= 13; c++) {
    Serial.print((pulseTime[c][0] + pulseTime[c][1] + pulseTime[c][2]) / 3);
    Serial.print("\t");
  }
  ...
  Serial.print("\tbounce FB: ");
  Serial.print(bounceFB);
  Serial.print("\tverifiedFieldSideFrontBack: ");
  Serial.print(verifiedFieldSideFrontBack);
  Serial.print("\tverifiedFieldSide: ");
  Serial.print(verifiedFieldSide);
  Serial.println();
...
void nanoCom() {
  ...
  //5 = Back Right
  //6 = Back
  if (1000 > pulseTimeTotal && haveBall == false) {
    DirectionGo = 0;
  }
...
void detectColor() {
  if (ColourIN >= 0 && ColourIN < 700 && onWhite != HIGH) { //White
  ...
  else if (ColourIN >= 700 && ColourIN <= 2000 && onDarkGreen != HIGH) { //Dark Green
    colorSensorError = LOW;
    onWhite = LOW;
    onDarkGreen = HIGH;
    onBlack = LOW;
  ...
  else if (ColourIN >= 2001 && ColourIN <= 2500 && onBlack != HIGH) { //Black
    colorSensorError = LOW;
    onWhite = LOW;
    onDarkGreen = LOW;
    onBlack = HIGH;
...
void IRCalc() {
  ...
  */
  ...
  pulseTimeSum[2] = 0;
  for (int y = 8; y <= 12; y++) {
    pulseTimeSum[0] += (pulseTime[y][0] + pulseTime[y][1] + pulseTime[y][2]) / 3;
  }
  for (int x = 2; x <= 5; x++) {
    pulseTimeSum[1] += (pulseTime[x][0] + pulseTime[x][1] + pulseTime[x][2]) / 3;
  }
  for (int z = 0; z <= 13; z++) {
    pulseTimeSum[2] += (pulseTime[z][0] + pulseTime[z][1] + pulseTime[z][2]) / 3;
  }
  pulseTimeTotal = pulseTimeSum[2];
  pulseTimeFrontTotal = pulseTimeSum[1];
  pulseTimeBackTotal = pulseTimeSum[0];


  for (int c = 0; c <= 13; c++) {
    pulseTimeCalc[c] = (pulseTime[c][0] + pulseTime[c][1] + pulseTime[c][2]) / 3;
  }
  ...
  for (int i = 0; i <= 13; i++) {
    ...
    if (pulseTime2[i] < micros() - 833 && tsopPowerState == HIGH) { //if 833increased to 3000 for power down microseconds have past with out a reading pulseTime =0. It's  is 833 microsecond between cycles. Pulses are merging together.
      pulseTime[i][2] = 0; pulseTime[i][1] = 0; pulseTime[i][0] = 0;
    }
  }
}
...
void Chaseball() {
  ...
  else if (TSOPnumberOUT == 3 || TSOPnumberOUT == 4 || TSOPnumberOUT == 13) {
    Direction = 1; //Forward
  }
...
void onWhiteFunc() {
  if (onWhite == HIGH) { //if on white
    if ((leftUltraSonic + rightUltraSonic) < 175 && abs(calibratedCompassValue) < 10 && (leftUltraSonic + rightUltraSonic) > 60) {
      if (rightUltraSonic < 25 && leftUltraSonic > 35) {
        verifiedFieldSide = 'R';
      }
      else if (leftUltraSonic < 25 && rightUltraSonic > 35) {
        verifiedFieldSide = 'L';
      }
    }
    if (abs(calibratedCompassValue) < 10 && (backUltraSonic + frontUltraSonic) < 230 && (backUltraSonic + frontUltraSonic) > 60) {
      if (backUltraSonic < 25 && frontUltraSonic > 35) {
        verifiedFieldSideFrontBack = 'B';
      }
      else if (frontUltraSonic < 25 && backUltraSonic > 35) {
        verifiedFieldSideFrontBack = 'F';
      }
    }
    if (frontUltraSonic < 33) {
      bounceFB = HIGH;
      verifiedFieldSideFrontBack = 'F';
    }
    else if (backUltraSonic < 33) {
      bounceFB = HIGH;
      verifiedFieldSideFrontBack = 'B';
    }
    disableMotorsButton();
    ...
    if (bounceFB == HIGH ) {
      if (verifiedFieldSideFrontBack == 'F') {
        delayOnWhite(50, 0); //Break For 100 milliseconds
        delayOnWhite(400, 6); //Goes Back For 400 milliseconds
        bounceFB = LOW;
      }
      else if (verifiedFieldSideFrontBack == 'B') {
        delayOnWhite(50, 0); //Break For 100 milliseconds
        delayOnWhite(400, 1); //Goes foward For 500 milliseconds
        bounceFB = LOW;
      }
    }
    else if (verifiedFieldSideFrontBack == 'F' && verifiedFieldSide == 'L') {
...
void haveBallLogic() {
  ...
  if (DirectionGo == 1 && kickerForwardTimer < millis() - 500) {
    if (pixyX >= 158 && pixyX <= 162 && digitalRead(23) == LOW && frontUltraSonic > 10 && motorsOn && abs(calibratedCompassValue) <= 90) {
      kickerForwardTimer = millis();
      kickerForwardReset = true;
      kicker = 27;
      nanoCom();
...
void haveBallCalc() {
  ...
  /////abcdefg123Serial.print(laser2On);
  if (laser1On == false && (pulseTime[13][2] + pulseTime[13][1] + pulseTime[13][0]) / 3 > 300 || (pulseTime[13][2] + pulseTime[13][1] + pulseTime[13][0]) / 3 > 300 && laser2On == false) { //if sensor 14 or 15 is getting a reading. pulseTime[13] > 350 ||
...
void CorrectDirection () {
  if (haveBall && abs(calibratedCompassValue) <= 90) {
    correctAmountMultiplier = 1;
    correctAmount = 100 - (abs(160 - pixyX) * correctAmountMultiplier);
    if (pixyX <= 160 && pixyX >= 160) {
      CorrectLeft = LOW;
      Straight = HIGH;
      CorrectRight = LOW;
...
void TSOP0() {
  if (tsopPowerState == HIGH) {
    ...
    if ( (pinState[0] == LOW)  ) {
      ...
      pulseTime[0][0] = pulseTime[0][1];
      pulseTime[0][1] = pulseTime[0][2];
      pulseTime[0][2] = pulseTime2[0] - pulseTime1[0];  //measure time between down and up
    }
  }
  else {
    pulseTime2[0] = micros();
...
void TSOP1() {
  if (tsopPowerState == HIGH) {
    ...
    if ( (pinState[1] == LOW)  ) {
      ...
      pulseTime[1][0] = pulseTime[1][1];
      pulseTime[1][1] = pulseTime[1][2];
      pulseTime[1][2] = pulseTime2[1] - pulseTime1[1]; //measure time between down and up
    }
  }
  else {
    pulseTime2[1] = micros();
...
void TSOP2() {
  if (tsopPowerState == HIGH) {
    ...
    if ( (pinState[2] == LOW)  ) {
      ...
      pulseTime[2][0] = pulseTime[2][1];
      pulseTime[2][1] = pulseTime[2][2];
      pulseTime[2][2] = pulseTime2[2] - pulseTime1[2] + 25; //measure time between down and up
    }
  }
  else {
    pulseTime2[2] = micros();
...
void TSOP3() {
  if (tsopPowerState == HIGH) {
    ...
    if ( (pinState[3] == LOW)  ) {
      ...
      pulseTime[3][0] = pulseTime[3][1];
      pulseTime[3][1] = pulseTime[3][2];
      pulseTime[3][2] = pulseTime2[3] - pulseTime1[3];  //measure time between down and up
    }
  }
  else {
    pulseTime2[3] = micros();
...
void TSOP4() {
  if (tsopPowerState == HIGH) {
    ...
    if ( (pinState[4] == LOW)  ) {
      ...
      pulseTime[4][0] = pulseTime[4][1];
      pulseTime[4][1] = pulseTime[4][2];
      pulseTime[4][2] = pulseTime2[4] - pulseTime1[4];  //measure time between down and up
    }
  }
  else {
    pulseTime2[4] = micros();
...
void TSOP5() {
  if (tsopPowerState == HIGH) {
    ...
    if ( (pinState[5] == LOW)  ) {
      ...
      pulseTime[5][0] = pulseTime[5][1];
      pulseTime[5][1] = pulseTime[5][2];
      pulseTime[5][2] = pulseTime2[5] - pulseTime1[5] + 10; //measure time between down and up
    }
  }
  else {
    pulseTime2[5] = micros();
...
void TSOP6() {
  if (tsopPowerState == HIGH) {
    ...
    if ( (pinState[6] == LOW)  ) {
      ...
      pulseTime[6][0] = pulseTime[6][1];
      pulseTime[6][1] = pulseTime[6][2];
      pulseTime[6][2] = pulseTime2[6] - pulseTime1[6];  //measure time between down and up
    }
  }
  else {
    pulseTime2[6] = micros();
...
void TSOP7() {
  if (tsopPowerState == HIGH) {
    ...
    if ( pinState[7] == LOW ) {
      ...
      pulseTime[7][0] = pulseTime[7][1];
      pulseTime[7][1] = pulseTime[7][2];
      pulseTime[7][2] = pulseTime2[7] - pulseTime1[7];  //measure time between down and up
    }
  }
  else {
    pulseTime2[7] = micros();
...
void TSOP8() {
  if (tsopPowerState == HIGH) {
    ...
    if ( (pinState[8] == LOW)  ) {
      ...
      pulseTime[8][0] = pulseTime[8][1];
      pulseTime[8][1] = pulseTime[8][2];
      pulseTime[8][2] = pulseTime2[8] - pulseTime1[8];  //measure time between down and up
    }
  }
  else {
    pulseTime2[8] = micros();
...
void TSOP9() {
  if (tsopPowerState == HIGH) {
    ...
    if ( (pinState[9] == LOW)  ) {
      ...
      pulseTime[9][0] = pulseTime[9][1];
      pulseTime[9][1] = pulseTime[9][2];
      pulseTime[9][2] = pulseTime2[9] - pulseTime1[9];  //measure time between down and up
    }
  }
  else {
    pulseTime2[9] = micros();
...
void TSOP10() {
  if (tsopPowerState == HIGH) {
    ...
    if ( (pinState[10] == LOW)  ) {
      ...
      pulseTime[10][0] = pulseTime[10][1];
      pulseTime[10][1] = pulseTime[10][2];
      pulseTime[10][2] = pulseTime2[10] - pulseTime1[10];  //measure time between down and up
    }
  }
  else {
...
void TSOP11() {
  if (tsopPowerState == HIGH) {
    ...
    if ( (pinState[11] == LOW)  ) {
      ...
      pulseTime[11][0] = pulseTime[11][1];
      pulseTime[11][1] = pulseTime[11][2];
      pulseTime[11][2] = pulseTime2[11] - pulseTime1[11];  //measure time between down and up
    }
  }
  else {
...
void TSOP12() {
  if (tsopPowerState == HIGH) {
    ...
    if ( (pinState[12] == LOW)  ) {
      ...
      pulseTime[12][0] = pulseTime[12][1];
      pulseTime[12][1] = pulseTime[12][2];
      pulseTime[12][2] = pulseTime2[12] - pulseTime1[12];  //measure time between down and up
    }
  }
  else {
...
void TSOP13() {
  if (tsopPowerState == HIGH) {
    ...
    if ( (pinState[13] == LOW)  ) {
      ...
      pulseTime[13][0] = pulseTime[13][1];
      pulseTime[13][1] = pulseTime[13][2];
      pulseTime[13][2] = pulseTime2[13] - pulseTime1[13];  //measure time between down and up
    }
  }
  else {
...
void ultrasonicDistanceSide() {
  if ((leftUltraSonic + rightUltraSonic) < 175 && abs(calibratedCompassValue) < 10) {
    if (rightUltraSonic > 82) {
      verifiedFieldSide = 'L';
    } else if (leftUltraSonic > 82) {
      verifiedFieldSide = 'R';
    }
  }
...
void ultrasonicDistanceFront() {
  ...
  */
  if ((frontUltraSonic + backUltraSonic) < 230) {
    if (frontUltraSonic > 110) {
      verifiedFieldSideFrontBack = 'B';
    }
    else if (backUltraSonic > 110) {
      verifiedFieldSideFrontBack = 'F';
    }
  }
