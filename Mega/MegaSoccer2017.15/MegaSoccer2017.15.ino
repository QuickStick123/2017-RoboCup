...
void onWhiteFunc() {
 if (onWhite == HIGH) { //if on white
    ...
    if (abs(calibratedCompassValue) < 10 && (backUltraSonic + frontUltraSonic) < 230 && (backUltraSonic + frontUltraSonic) > 60) {
      ...
      if (frontUltraSonic < 25 && (leftUltraSonic + rightUltraSonic) < 60 && abs(calibratedCompassValue) < 10) {
        bounceFB = HIGH; 
        verifiedFieldSideFrontBack = 'F';
      }
      else if (backUltraSonic < 25 && (leftUltraSonic + rightUltraSonic) < 60 && abs(calibratedCompassValue) < 10) {
        bounceFB = HIGH; 
        verifiedFieldSideFrontBack = 'B';
      }
...
void ultrasonicDistanceFront() {
  ...
  */
  if ((frontUltraSonic + backUltraSonic) < 230 && abs(calibratedCompassValue) < 10) {
    if (frontUltraSonic > 110) {
      verifiedFieldSideFrontBack = 'B';
    } else if (backUltraSonic > 115) {
      verifiedFieldSideFrontBack = 'F';
    }
  }
