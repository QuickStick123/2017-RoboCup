...
void nanoCom(){
  /*if(I2CComCooldown <= millis()-10){
    ...
    if(correctAmount > 28) 
      correctAmount = 28; //Setting 30 as the maximum adjustemnt.
    correctAmount = correctAmount + 2; //Adding 2 to override motor power differences.
    Wire.write(correctAmount);
    Wire.write(kicker);
    Wire.write(Identifier + correctAmount + DirectionGo * motorsOn + kicker); //sum of numbers used to help filter out bad dat
    kicker = false;
...
void CorrectDirection () {
  if (calibratedCompassValue >=-0.5 && calibratedCompassValue <= 0.5) {
  CorrectRight = LOW;
 CorrectLeft = LOW;
 Straight = HIGH;
