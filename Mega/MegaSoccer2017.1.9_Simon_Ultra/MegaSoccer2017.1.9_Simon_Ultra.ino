...
boolean fieldSide;
volatile boolean verifiedFieldSide;
boolean fieldSideFrontBack;
volatile boolean verifiedFieldSideFrontBack;
...
void nanoCom() {
  ...
  /*correctAmount = abs(calibratedCompassValue);
    if (correctAmount > 60)
...
void haveBallCalc() {
  Serial.print("\tlaser1On: ");
  Serial.print(laser1On);
  Serial.print("\tlaser2On: ");
  Serial.print(laser2On);
...
// field side 1 is right
  ...
  if(rightUltraSonic + leftUltraSonic >= 155 && rightUltraSonic + leftUltraSonic <= 175){
    if(rightUltraSonic > leftUltraSonic){
      fieldSide = false;
      verifiedFieldSide = false;
    }
    else if(rightUltraSonic < leftUltraSonic){
      fieldSide = true;
      verifiedFieldSide = true;
    }
  }else{
    if(rightUltraSonic > leftUltraSonic){
      fieldSide = false;
  ...
  Serial.print("Field Side: ");
  Serial.print(fieldSide);
 }
...
// field side 1 is right
  ...
  if(frontUltraSonic + backUltraSonic >= 160 && frontUltraSonic + backUltraSonic <= 180){
    if(frontUltraSonic > backUltraSonic){
      fieldSideFrontBack = false;
      verifiedFieldSideFrontBack = false;
    }
    else if(frontUltraSonic < backUltraSonic){
      fieldSideFrontBack = true;
      verifiedFieldSideFrontBack = true;
    }
  ...
  Serial.print("Verified Field Side FrontBack: ");
  Serial.println(verifiedFieldSideFrontBack);
  Serial.print("Field Side FrontBack: ");
  Serial.print(fieldSideFrontBack);
 }
void bounceOnWhite() {
  if(onWhite){
    if(fieldSide){
      if(true){
        Serial.print("Bounce On White");
        DirectionGo = 0;
        nanoCom();
        Serial.print("Bounce On White     2    ");
        delay(100);
        Straight = HIGH;
        Serial.print("Bounce On White     3    ");
        CorrectRight = LOW;
        CorrectLeft = LOW;
        DirectionGo = 2;
        Serial.print("Bounce On White     4    ");
        nanoCom();
        bouceOnWhiteLoop();
        Serial.print("Bounce On White     10    ");
      }
    }
  }
}
void bouceOnWhiteLoop() {
  int startTime;
  Serial.print("Bounce On White     6    ");
  startTime = millis();
  while(millis() - startTime < 4000){
    Serial.print("Bounce On White     7    ");
