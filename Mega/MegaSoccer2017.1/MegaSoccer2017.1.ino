...
boolean onBlack = LOW;
boolean onLightGreenLeft = LOW;
boolean onLightGreenRight = LOW;
int pastOnWhite =3;
...
// Set direction ports for the motor driver
...
if (onLightGreenLeft == HIGH) {
  Serial.print("LightGreenLeft");
}
if (onLightGreenRight == HIGH) {
  Serial.print("LightGreenRight");
}
...
//
if(runOnce == LOW){
  startCompassReading = compassValue;
  runOnce = HIGH;
  delay(10);
  }
}
...
void nanoCom(){
  ...
  Wire.write(DirectionGo); // sends one byte DirectionSend
  Identifier = Identifier + DirectionGo;
  if(drehM1+100 != 100){
    Wire.write(drehM1 + 100); // Motor1Power
    Serial.print("\tdrehM1: ");
    Serial.print(drehM1);
    Identifier = Identifier + abs(drehM1);
  }
    if(drehM2+100 != 100){
      Wire.write(drehM2 + 100); // Motor2Power
      Serial.print("\tdrehM2: ");
      Serial.print(drehM2);
      Identifier = Identifier + abs(drehM2);
  }
    if(drehM3+100 != 100){
      Wire.write(drehM3 + 100); // Motor3Power
      Serial.print("\tdrehM3: ");
      Serial.print(drehM3);
      Identifier = Identifier + abs(drehM3);
  }
  Serial.print("\tIdentifier: ");
  Wire.write(Identifier); //Identifier
  Serial.print(Identifier);
  Identifier = 0;
  Wire.endTransmission();    // stop transmitting
}
void detectColor(){
  if(ColourIN >= 0 && ColourIN < 200 && onWhite != HIGH){
  colorSensorError = LOW;
  onWhite = HIGH;
  onDarkGreen = LOW;
  onBlack = LOW;
  onLightGreenLeft = LOW;
  onLightGreenRight = LOW;
  colorTimer = micros();
  }
  else if(ColourIN >= 200 && ColourIN <= 500){
  colorSensorError = LOW;
  onWhite = LOW;
  onDarkGreen = LOW;
  onBlack = LOW;
  if(pastOnWhite == 2){
    if(directionGoing == 3 || 5 && onLightGreenLeft != HIGH){
      onLightGreenLeft = HIGH;  
      onLightGreenRight = LOW;
      colorTimer = micros();
    }
    else if(directionGoing == 2 || 4 && onLightGreenRight != HIGH){
      onLightGreenRight = HIGH;
      onLightGreenLeft = LOW;
      colorTimer = micros();
    }
  }
  else{
  if( directionGoing == 3 || 5 && onLightGreenRight != HIGH){
    onLightGreenRight = HIGH;
    onLightGreenLeft = LOW;
    colorTimer = micros();
  }
  else if(directionGoing == 2 || 4 && onLightGreenLeft != HIGH){
    onLightGreenLeft = HIGH;
    onLightGreenRight = LOW;
    colorTimer = micros();
  }
  ...
  else if(ColourIN >= 500 && ColourIN <= 1000 && onDarkGreen != HIGH){
  colorSensorError = LOW;
  onWhite = LOW;
  onDarkGreen = HIGH;
  onBlack = LOW;
  ...
  else if(ColourIN >= 1001 && ColourIN <= 2000 && onBlack != HIGH){
  colorSensorError = LOW;
  onWhite = LOW;
  onDarkGreen = LOW;
  onBlack = HIGH;
