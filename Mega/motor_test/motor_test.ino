...
void turnM1(int velo){
  if(false){ // If input is 0, power to motors is set to full but both in directions to act as brakes
    analogWrite(SpeedPort1, 255); 
    digitalWrite(DirectionPort11, HIGH);
    digitalWrite(DirectionPort12, HIGH);
  }
  else if(true){ // If input is grater than 0, power to motors is set to input and set to turn in one direction
    velo = velo * 2.55;
...
void forward() {
 if (true) {// Go forward 
    turnM2 (100);
    turnM1 (100);
  }else if (correct == 8) { // Correct Left, puts less power to motor 1
    turnM1 (8*speedMulitply);
    turnM2 (10*speedMulitply);
  }else if (correct == 9) {// Correct Right, puts less power to motor 2
    turnM1 (10*speedMulitply);
    turnM2 (8*speedMulitply);
  }
  turnM3 (0);
}
