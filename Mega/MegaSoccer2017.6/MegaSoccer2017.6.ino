...
pinMode(tsopPowerPin2, OUTPUT);
digitalWrite(tsopPowerPin, HIGH);
digitalWrite(tsopPowerPin2, HIGH);
tsopPowerState = HIGH;
tsopTime = micros();
...
void nanoCom(){
  /*if(I2CComCooldown <= millis()-10){
    ...
    Wire.write(kicker);
    kicker = false;
    Wire.write(Identifier + DirectionGo* motorsOn + kicker); //sum of numbers used to help filter out bad data
...
void CorrectDirection () {
  if (calibratedCompassValue >=-2.7 && calibratedCompassValue <= 2.7) {
  CorrectRight = LOW;
  CorrectLeft = LOW;
  Straight = HIGH;
...
if ((tsopTime+100000) < micros() and  tsopPowerState == HIGH){
    ...
    digitalWrite(tsopPowerPin2, LOW);
    tsopPowerState = LOW;
