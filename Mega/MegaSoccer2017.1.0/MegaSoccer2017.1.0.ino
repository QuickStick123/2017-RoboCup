...
void nanoCom(){
  /*if(I2CComCooldown <= millis()-10){
    ...
    Wire.write(kicker);
    Wire.write(Identifier + DirectionGo * motorsOn + kicker); //sum of numbers used to help filter out bad dat
    kicker = false;
...
void CompassCalculate(){
  ...
  headingRadians = atan2(calibrated_values[0], calibrated_values[1]);
  if(headingRadians < 0)
    headingRadians += 2*PI;
